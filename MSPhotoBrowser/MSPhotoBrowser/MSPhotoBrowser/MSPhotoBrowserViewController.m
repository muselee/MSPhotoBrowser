...
- (void)viewDidLoad{
    ...
    // 获取图片总数
    if ([self.ms_dataSource conformsToProtocol:@protocol(MSPhotoBrowserDataSource)] &&
...
- (MSPhotoScrollerViewController *)imageScrollerViewControllerForPage:(NSInteger)page {
    ...
    if (self.ms_delegate &&
        ...
        //顶部view
        CGFloat  topHeight = defaultTopBottomViewHeight;
        if ([self.ms_delegate respondsToSelector:@selector(viewController:heightForTopViewAtIndex:)]) {
            
            topHeight = [self.ms_delegate viewController:self heightForTopViewAtIndex:page];
        }
        ...
        //底部view
        CGFloat  bottomHeight = defaultTopBottomViewHeight;
        if ([self.ms_delegate respondsToSelector:@selector(viewController:heightForBottomViewAtIndex:)]) {
            
            bottomHeight = [self.ms_delegate viewController:self heightForBottomViewAtIndex:page];
        }
        ...
        if ([self.ms_delegate respondsToSelector:@selector(viewController:bottomViewForPageAtIndex:)]) {
            ...
            frame.size.width = self.view.bounds.size.width;
            frame.origin.y = self.view.bounds.size.height - bottomHeight;
